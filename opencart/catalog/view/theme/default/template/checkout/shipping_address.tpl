<form class="form-horizontal">
  ...
  <div class="radio">
    <label>
      <input type="radio" name="shipping_address" value="new_NP" />
      <?php echo $text_address_new_NP; ?></label>
  </div>
  <div id="shipping-new-NP" style="display: <?php echo ($addresses ? 'none' : 'block'); ?>;">
    <select name="choose_city_NP" id="choose-city-NP" class="form-control">
      <option value="NULL"><?php echo $text_choose_city_NP; ?></option>
      <?php foreach ($cities as $key => $city) { ?>
      <option value="<?php echo $city; ?>" ref="<?php echo $refs[$key]; ?>"><?php echo $city; ?></option>
      <?php } ?>
    </select>
  </div>
  <div id="shipping-new-NP-dep" style="display: <?php echo ($addresses ? 'none' : 'block'); ?>;">
    <select name="choose_city_NP_dep" id="choose-city-NP-dep" class="form-control">
      <option value="NULL"><?php echo $text_choose_city_NP_dep; ?></option>
    </select>
  </div>
  <?php } ?>
...
$('input[name=\'shipping_address\']').on('change', function() {
	if (this.value == 'new') {
		...
		$('#shipping-new').show();
        $('#shipping-new-NP').hide();
        $('#shipping-new-NP-dep').hide();
	} else if (this.value == 'new_NP'){
		$('#shipping-existing').hide();
		$('#shipping-new').hide();
        $('#shipping-new-NP').show();
        $('#shipping-new-NP-dep').hide();
	} else {
    ...
    $('#choose-city-NP').on('change',  function() {

        //alert($('#choose-city-NP option:selected').attr('ref'))
       //alert($('#choose-city-NP').val())
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    "methodProperties": {
                        "CityName": $('#choose-city-NP').val(),
                        "CityRef": $('#choose-city-NP option:selected').attr('ref')
                    },
                    "apiKey": "9dfdc9e0ede872f1de913dd7d352ec00"
                }),
                success: function(data) {
                    $('#shipping-new-NP-dep').show();
                    for (var i =0; i < data.data.length; i++) {
                        $('#choose-city-NP-dep').append('<option value = "' +  data.data[i].DescriptionRu + '">' +  data.data[i].DescriptionRu + '</option>')
                      //  document.write(data.data[i].DescriptionRu + "<br\>");
                    }
